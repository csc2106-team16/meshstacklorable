/*
 * M5StickC Plus — Integrated Air Quality Monitor (RECEIVER NODE)
 * Combines: WiFi/MQTT (TLS via gateway) + BLE Broadcasting + LoRa Fallback
 *
 * THIS NODE HAS NO SENSOR — it only receives and displays data
 * from other nodes via MQTT (WiFi mode) or LoRa (fallback mode).
 *
 * UART COMMANDS (M5Stick → Arduino):
 *   "LORA,ON\n"   — tells Arduino to start LoRa RX/relay
 *   "LORA,OFF\n"  — tells Arduino to stop LoRa RX/relay
 *
...
// Node identity
#define MY_NODE_ID        2
#define BLE_DEVICE_NAME   "ENV-NODE-02"

// WiFi credentials
#define WIFI_SSID         "iPhone"
#define WIFI_PASSWORD     "MIJAWIFI78"

// Smoke threshold — must match sender Arduino's threshold
...
// ===============================================================
// SENSOR DATA (received from other nodes)
// ===============================================================
...
// ###############################################################
//  WIFI MODE — RECEIVE ONLY + BLE + DISPLAY
// ###############################################################
void loopWiFiMode() {
  // NO MQTT publish — this node has no sensor, only receives

  // BLE broadcast (rebroadcasts whatever data we last received)
...
void connectMQTT() {
  ...
  // Subscriptions handled in gateway.cpp reconnMQTT()
}
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  // Skip heartbeat and /lv topics
  if (topicStr.indexOf("heartbeat") >= 0) return;
  if (topicStr.endsWith("/lv")) return;

  // Skip our own device's messages
  if (topicStr.indexOf("m5stick_node" + String(myNodeId)) >= 0) return;

  // Format 1: raw node data "N<id>,R<raw>,V<voltage>,<STATUS>"
  if (msg.startsWith("N")) {
    int n1 = msg.indexOf(',');
    if (n1 > 0) {
      ...
      if (sourceNode != myNodeId) {
        parseUART(msg);
        Serial.print("[MQTT] Got node data from node ");
  ...
  // Format 2: Ubidots smokesensor JSON
  // e.g. topic: /v1.6/devices/m5stick_node1/smokesensor_4
  //      msg:   {"value":87.0,"timestamp":...}
  if (topicStr.indexOf("smokesensor") >= 0 && msg.indexOf("\"value\"") >= 0) {
    // Extract source node from topic
    int nodeIdx = topicStr.indexOf("m5stick_node");
    int sourceNode = 0;
    if (nodeIdx >= 0) {
      String nodeStr = topicStr.substring(nodeIdx + 12);
      sourceNode = nodeStr.toInt();
    }

    // Skip our own
    if (sourceNode == myNodeId) return;
    ...
    if (valIdx >= 0) {
      ...
      if (commaIdx > 0) valStr = valStr.substring(0, commaIdx);
      valStr.trim();
      int smokeVal = (int)valStr.toFloat();

      rawValue = smokeVal;
      voltage = smokeVal * (5.0 / 1023.0);
      smokeDetected = (smokeVal >= SMOKE_THRESHOLD);
      sensorValid = true;
      dataReceived = true;
      fromNodeId = sourceNode > 0 ? sourceNode : 1;

      Serial.printf("[MQTT] Node %d smoke: %d %s\n",
                    fromNodeId, smokeVal, smokeDetected ? "ALERT" : "CLEAR");
    }
  }
}
